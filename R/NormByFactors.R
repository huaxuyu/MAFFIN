#' Normalization by providing normalization factors.
#'
#' @description
#' Sample normalization by providing normalization factors.
#'
#' @param FeatureTable Data frame with features in row and samples in column (default).
#' @param NormFactors A vector representing the normalization factors.
...
#'
#' @examples
#' factorNormedTable = NormByFactors(TestingData, MAFFINNorm(TestingData)$NormFactor)


NormByFactors = function(FeatureTable, NormFactors, IntThreshold=0, SampleInCol=TRUE,
                         output=FALSE, OutputNormFactors=FALSE, RunEvaluation=TRUE, bwOpt=NA){
  ...
  for (i in 1:ncol(sample_table)) {
    FeatureTable[-1,FeatureTable_index[i]] = as.numeric(FeatureTable[-1,FeatureTable_index[i]]) / NormFactors[i]
  }
  ...
  if (OutputNormFactors) {
    ...
    cat(round(NormFactors, digits = 3))
  }
  results = list(FeatureTable, NormFactors)
  names(results) = c("NormedTable", "NormFactor")
  if (RunEvaluation) {
    results = list(FeatureTable, NormFactors, pRMAD_each1, pRMAD_each2)
